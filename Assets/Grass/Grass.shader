...
Shader "Custom/GeometryGrass"
{
	...
	Properties
	{
		...
		_MinColorDistance("Min Color Distance", Float) = 40
		_MaxDist("Max Distance", Float) = 60
		_LighteningStrength("Lightening Strength", Float) = .05
		_LighteningColor("Lightening Color", Color) = (1,1,1,1)
	}
...
#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Shadows.hlsl"	
	...
	float _MinDist, _MaxDist, _MinColorDistance, _LighteningStrength;
	...
	float4 _TopColor;
	float4 _BottomColor;
	float4 _LighteningColor;
	float _AmbientStrength;

	float remap(float value, float startLow, float startHigh, float endLow, float endHigh){
	...
	float distanceFromCamera = distance(i.worldPos, _WorldSpaceCameraPos);

	float lighteningFactor = remap(distanceFromCamera, _MinColorDistance, _MaxDist, 0, _LighteningStrength);
	final += lighteningFactor * _LighteningColor;
	return final;
   }
	   ENDHLSL
   }
